...
mongoose.connect(connectionString, { useNewUrlParser: true, useUnifiedTopology: true })
  .then(() => console.log(`Data Fetcher (${Date.now()}): Successfully connected to the MongoDB database`))
  .catch(err => {
    console.error(`Data Fetcher (${Date.now()}): An error occurred while connecting to the MongoDB database using '${connectionString}'`);
...
const fetchTokenBalance = async (token, address, type = "balance") => {
  ...
  if (balance === undefined) {
    ...
    throw new Error(`Operation Error (${Date.now()}): Missing '${type}' for token '${token}' in response`);
  };
...
const fetchTimestamp = async (address) => {
  ...
  if (timestamp === undefined) {
    ...
    throw new Error(`Operation Error (${Date.now()}): Missing 'burn_block_time' in response`);
  };
...
const fetchAllData = async () => {
  try {
    ...
    data.oracle.timestamp.source = await fetchTimestamp("SP2YYZRZ210D7CPG5M8TDJE8V694B6AVGAKM99MH9");
    data.oracle.timestamp.proxy = await fetchTimestamp("SP32BWYT80264254QRAB4ZZSR1W14VTKSQ92Y1GZ3");
    ...
    await model.save();
    console.log(`Data Fetcher (${Date.now()}): Successfully saved data to the MongoDB database`);

    await mongoose.disconnect();
    console.log(`Data Fetcher (${Date.now()}): Successfully disconnected from the MongoDB database`);
  } catch (err) {
